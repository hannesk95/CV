function [mask] = segmentation(left,right)
  ...
  masks = zeros(img_height, img_width, N);
  for k = 1:(N-1)          
      %% Find region of interest
      [~, top_left_roi, bottom_right_roi] = find_roi(scaled_left(:,:,1:3), scaled_left(:,:,(1:3) + k*3), [80, 60]);
      ...
      scaled_left_roi = scaled_left(top_left_roi(2):bottom_right_roi(2),top_left_roi(1):bottom_right_roi(1), :);
      scaled_right_roi = scaled_right(top_left_roi(2):bottom_right_roi(2),top_left_roi(1):bottom_right_roi(1), :);

      %% Search for a more detailed ROI
      tile_w = 5;
      tile_h = 5;
      [roi_detail, ~, ~] = find_roi(scaled_left_roi(:,:,1:3), scaled_left_roi(:,:,(1:3) + k*3), [tile_w tile_h], 0);
      ...
      mask_detail = zeros(ceil([size(scaled_left_roi, 1) / scaling_factor, size(scaled_left_roi, 2) / scaling_factor]));
      ...
      translation = top_left_detail - [1 1] + (top_left_roi - [1 1]) / scaling_factor;
